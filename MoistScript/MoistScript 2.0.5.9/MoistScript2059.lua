...
file:write(tostring("MoistScript=2.0.5.9") .. "\n")
...
Paths, Settings, threads, kick_param, tele_presets = {}, {}, {}, {}, {}
...
msg2 =  msg2 or "MoistScript 2.0.5.9"
...
function VersionCheck()
	...
	Load_Settings()
if Settings["MoistScript"] ~= "2.0.5.9" then
...
Orbital_Proxy_Thread = function(pid)
	...
	if player.get_player_scid(pid) ~= -1 then
...
--local Menu Features
globalFeatures.parent = menu.add_feature("MoistScript 2.0.5.9", "parent", 0).id
...
PlayerFeatParent = menu.add_player_feature("Moists Script 2.0.5.9", "parent", 0).id
...
function Save_Preset(name)
	...
	SaveTeleports()
		...
		Teleport_Features[#Teleport_Features+1] = menu.add_feature(s, "action_value_str", globalFeatures.self_Tele, function(feat)
	local pos = v3()
	local presetid
	presetid = s
	...
	entity.set_entity_coords_no_offset(pped, pedpos)
	end
	
	
	elseif feat.value == 1 then
	Remove_Teleport(s)
	Teleport_Features[#Teleport_Features].hidden = true
	
	end
end)
Teleport_Features[#Teleport_Features]:set_str_data({"Teleport","Delete"})
...
Teleport_Features[#Teleport_Features+1] = menu.add_feature(k, "action_value_str", globalFeatures.self_Tele, function(feat)
	pos = v3()
	 pos = v
	 presetid = k
	if feat.value == 0 then
...
impact_orbstrike = menu.add_feature("Impact Orbital Strike", "toggle", globalFeatures.self_combat_Ped, function(feat)
    ...
    if feat.on then
		...
		return HANDLER_CONTINUE
			...
			graphics.start_networked_ptfx_non_looped_at_coord("scr_xm_orbital_blast", pos, v3(0, 0, 0), 5.000, false, false, true)
...
function BruteForce_RemoveBlips()
	...
	local ObjEntitys = objects.get_all_objects()
...
startup = menu.add_feature("int MoistScript", "toggle", 0, function(feat)
	if not feat.on then
	return HANDLER_POP
	end
	if InstallState == "Pass" then
	if utils.time() - YieldCount > 19 then
