...
local FilesCheck = {"\\scripts\\Moists Modder_Module.lua","\\scripts\\MoistsLUA_cfg\\interiors.lua","\\scripts\\MoistsLUA_cfg\\Moist_KickParam.ini","\\scripts\\MoistsLUA_cfg\\Moist_Kicks.ini","\\scripts\\MoistsLUA_cfg\\online_interiors.lua"}
...
file:write(tostring("MoistScript=2.0.6.0") .. "\n")
...
msg2 =  msg2 or "MoistScript 2.0.6.0"
...
Load_Settings()
if Settings["MoistScript"] ~= "2.0.6.0" then
...
--TODO: Orbital Room Proximity Detection
...
--local Menu Features
globalFeatures.parent = menu.add_feature("MoistScript 2.0.6.0", "parent", 0).id
...
Moist_TestMode = menu.add_feature("Enable TestMode Functions", "toggle", globalFeatures.moist_tools.id, function(feat)
local debug_mode
...
Dump = function(o)
	if type(o) == 'table' then
		local s = 'Table Dump: ' .. o .. tostring(o) .. '{}\n{'
...
if feat.on then
	...
	if Active_menu ~= nil then
		...
		if realcheck == true and id ~= nil then
			...
			text5 = "\n~h~~o~Interior:~w~ " .. tostring(Players[pid].Interiorname) ..  "\t | 2D = " .. tostring(dist2D) .."\t | 3D = " .. tostring(dist3D)
...
PlayerFeatParent = menu.add_player_feature("Moists Script 2.0.6.0", "parent", 0).id
...
function Script_Event_Hooks()
	...
	function Get_Missile_Entity()
		...
		for i = 1, #MissEntity do
			...
			if hash == 1262355818 then
				...
				ui.set_blip_sprite(MissileBlip[#MissileBlip], 368)
				-- 368 / 548
	...
	--TODO: Player RC Vehicle Detection
...
ScriptLocals["playerlist"] = function()
	...
	--TODO: Standard Player Loop function
			...
			for pid = 0, 31 do
				...
				if scid ~= -1 then
					...
					if Online then	
						...
						if player.is_player_god(pid) and not Players[pid].isRC and Players[pid].isint ~= true and Players[pid].PedSpawned == true  then
						...
						if player.is_player_vehicle_god(pid) and Players[pid].isint ~= true and Players[pid].PedSpawned == true  then
							...
							elseif player.is_player_vehicle_god(pid) and  Players[pid].isint ~= true and Players[pid].PedSpawned == true then
	...
	--TODO: Experimental Loop Function
