...
local task_ids = {
	...
	["CTaskMoveSeekEntity_CEntitySeekPosCalculatorLastNavMeshIntersection1"]=211,
	["CTaskMoveSeekEntity_CEntitySeekPosCalculatorLastNavMeshIntersection2"]=212,
	["CTaskMoveSeekEntity_CEntitySeekPosCalculatorXYOffsetFixed1"]=213,
...
function data.get_AI_task_id(task_name)
	if task_ids[task_name] then
		return tonumber(task_ids[task_name])
	else
		return nil
	end
end

--[[--NOT WORKING (task_ids populate good but i cant retrieve data from it, looks like the table index is not correct, idk)
function Import_task_ids()
	if utils.file_exists(path_taskIDsTXT) then
		local file = io.open(path_taskIDsTXT, "rb")
		local str = ""
		...
		for line_Saved in str:gmatch("([^\n]*)\n+") do
			
			--Regex detect if 'line_Saved' string start by "(somthing)-"
			local taskid_fromfile = line_Saved:match("^(.+)-")
			if tonumber(taskid_fromfile) then
					--Regex detect : retrieve a certain data after the name "=(somthing)"
					local Saved_data_from_file = line_Saved:match("-(.+)")
					local Retrieved_data = nil
					--if this data gives a valid number and that the declared setttings is a number, convert Retrieved_data to number
					if tostring(Saved_data_from_file) then
						Retrieved_data = tostring(Saved_data_from_file)
					end
					if tonumber(Saved_data_from_file) then
						Retrieved_data = tonumber(Saved_data_from_file)
					end
					Anger_notify("i'm inXX"..tostring(taskid_fromfile).."XX"..Retrieved_data.."XX", "green")
					--Import the saved settings to the running config
					if Retrieved_data then
						Anger_notify("i'm saving"..tostring(taskid_fromfile), "green")
						task_ids[tostring(Retrieved_data)] = tonumber(taskid_fromfile)
						Anger_notify("savedXX"..tostring(Retrieved_data).."XX"..tostring(task_ids[tostring(Retrieved_data)]).."XX", "green")
					end
		...
		ui.notify....("exemple".."CTaskHandsUp:"..tostring(task_ids[tostring("CTaskHandsUp")]),"green")
