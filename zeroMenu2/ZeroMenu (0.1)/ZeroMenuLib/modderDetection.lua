    ...
    for slot = 0, 31 do
      if player.get_player_scid(slot) ~= -1 and player.get_player_scid(slot) ~= 4294967295 then
        if checkedList[player.get_player_name(slot)] == nil then        
          ...
          --Impulse Spoof IP 153.31.113.27, FBI check
          if(ipBlockA == 153 and ipBlockB == 31 and ipBlockC == 113 and ipBlockD == 27) then
            ui.notify_above_map(player.get_player_name(slot) .. " has an invalid IP " .. ipS .. " Impulse","ZModder Detection",140)
          end
