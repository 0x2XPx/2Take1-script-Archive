...
--entity.set_entity_max_speed(ped.get_vehicle_ped_is_using(player.get_player_ped(player.player_id())),1)
              ...
              if entity.get_entity_model_hash(veh) == 2069146067 and oppressor.on then
                ...
                ui.notify_above_map("Trying to stop Oppressor MK2 with player " .. player.get_player_name(slot) ,"Anti Depressor",140)
              ...
              --akula
              if entity.get_entity_model_hash(veh) == VehicleHash. and akula.on then
                if not network.has_control_of_entity(veh) then
                  network.request_control_of_entity(veh)  
                end   
                entity.set_entity_max_speed(veh,100000)
              ...
              --mk1
              if entity.get_entity_model_hash(veh) == 884483972 and mk1.on then
                if not network.has_control_of_entity(veh) then
                  network.request_control_of_entity(veh)  
                end   
                entity.set_entity_max_speed(veh,100000)
                ...
                entity.set_entity_rotation(veh,rotation)
                vehicle.set_vehicle_forward_speed(veh, newSpeed)                
                ui.notify_above_map("Trying to stop Oppressor MK1 with player " .. player.get_player_name(slot) ,"Anti Depressor",140)
                if max_speed_grief.on then                  
                  system.wait(2000)
                  if not network.has_control_of_entity(veh) then
                    network.request_control_of_entity(veh)  
