...
function Ghost.isKeyDown(keyNum)
	local key = MenuKey()
	key:push_vk(keyNum)
	return key:is_down()
end
...
function Ghost.doWrite(time, key, extraArg, ignoreMax, stringTable, functionToDo)
	if Ghost.isKeyDown(key) then
		if (Ghost.isKeyDown(key) and extraArg and #stringTable.string < stringTable.maxLen) or (Ghost.isKeyDown(key) and ignoreMax) then
			functionToDo()
		end
		local timer = utils.time_ms() + time
		while timer > utils.time_ms() and Ghost.isKeyDown(key) and extraArg do
			system.wait(0)
		end
		while timer < utils.time_ms() and Ghost.isKeyDown(key) and extraArg do
...
function Ghost.getInput(title, default, len, inputType, dataTable)
	...
	local EBreak = menu.create_thread(function()
		while true do
			if Ghost.isKeyDown(0x10) and Ghost.isKeyDown(0x1B) then
	...
	while true do
		...
		for k, v in pairs(Ghost.charCodes[inputType]) do
			Ghost.doWrite(500, v[1], not Ghost.isKeyDown(0x11), false, stringTable, function()
				if not Ghost.isKeyDown(0x10) then
		...
		elseif Ghost.isKeyDown(0x1B) then
			if not Ghost.isKeyDown(0x10) then
				while Ghost.isKeyDown(0x1B) do
					for i = 1, 20 do
		...
		elseif Ghost.isKeyDown(0x11) and Ghost.isKeyDown(0x56) then
			...
			while Ghost.isKeyDown(0x11) and Ghost.isKeyDown(0x56) do
				system.wait(0)
			end

		elseif Ghost.isKeyDown(0x11) and Ghost.isKeyDown(0x43) then
			utils.to_clipboard(stringTable.string)
			while Ghost.isKeyDown(0x11) and Ghost.isKeyDown(0x43) do
				system.wait(0)
			end
		end

		Ghost.doWrite(500, 0x08, true, true, stringTable, function()
			if Ghost.isKeyDown(0x11) and Ghost.isKeyDown(0x08) then
				stringTable.string = ""
