...
local miscdata = {
    fakeinvites = {'Yacht', 'Office', 'Clubhouse', 'Office Garage', 'Custom Auto Shop', 'Apartment'},
    ceomoney = {
...
local MainThread1 = menu.create_thread(function()
        ...
        while f.on do
            if ped.is_ped_shooting(get.OwnPed()) then
                ...
                if ped.is_ped_a_player(pl) then
                    ...
                    scriptevent.kick(player.get_player_from_ped(pl))
                end
            end
            coroutine.yield(0)
        end
    ...
    Script.Feature['Lobby Fake Invite Spam'] = menu.add_feature('Fake Typing Indicator', 'toggle', Script.Parent['Lobby Trolling'], function(f)
        while f.on do
            for id = 0, 31 do
                if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                    scriptevent.Send('Typing Begin', {player.player_id(), 0, math.random(0, 10000)}, id)
                end
        ...
        for id = 0, 31 do
            if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                scriptevent.Send('Typing Stop', {player.player_id(), 0, math.random(0, 10000)}, id)
            end
    ...
    Script.Feature['Lobby Cash Removed'] = menu.add_feature('Cash Removed', 'action_value_str', Script.Parent['Lobby Notifications'], function(f)
        ...
        if f.value == 0 then
            for id = 0, 31 do
                if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                    scriptevent.Send('Notification', {player.player_id(), 689178114, math.random(-2147483647, 2147483647)}, id)
                end
        ...
        for id = 0, 31 do
            if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                scriptevent.Send('Notification', {player.player_id(), 689178114, amount}, id)
            end
    ...
    Script.Feature['Lobby Cash Stolen'] = menu.add_feature('Cash Stolen', 'action_value_str', Script.Parent['Lobby Notifications'], function(f)
        if f.value == 0 then
            for id = 0, 31 do
                if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                    scriptevent.Send('Notification', {player.player_id(), 2187973097, math.random(-2147483647, 2147483647)}, id)
                end
        ...
        for id = 0, 31 do
            if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                scriptevent.Send('Notification', {player.player_id(), 2187973097, amount}, id)
            end
    ...
    Script.Feature['Lobby Cash Banked'] = menu.add_feature('Cash Banked', 'action_value_str', Script.Parent['Lobby Notifications'], function(f)
        if f.value == 0 then
            for id = 0, 31 do
                if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                    scriptevent.Send('Notification', {player.player_id(), 1990572980, math.random(-2147483647, 2147483647)}, id)
                end
        ...
        for id = 0, 31 do
            if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                scriptevent.Send('Notification', {player.player_id(), 1990572980, amount}, id)
            end
    ...
    Script.Parent['Lobby Teleports'] = menu.add_feature('Teleports', 'parent', Script.Parent['Lobby Trolling'], nil).id


    Script.Feature['Lobby Random Apartment Invite'] = menu.add_feature('Random Apartment Invite', 'action', Script.Parent['Lobby Teleports'], function()
        for id = 0, 31 do
            if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
    ...
    Script.Feature['Lobby Apartment Invite Loop'] = menu.add_feature('Apartment Invite Loop', 'toggle', Script.Parent['Lobby Teleports'], function(f)
        while f.on do
            for id = 0, 31 do
                if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                    scriptevent.Send('Apartment Invite', {player.player_id(), id, 1, 0, math.random(1, 114), 1, 1, 1}, id)
    ...
    Script.Feature['Lobby Warehouse Invite'] = menu.add_feature('Warehouse Invite', 'action_value_str', Script.Parent['Lobby Teleports'], function(f, id)
        if f.value == 22 then
            for id = 0, 31 do
                if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                    scriptevent.Send('Warehouse Invite', {player.player_id(), 0, 1, math.random(1, 22)}, id)
    ...
    Script.Feature['Lobby Warehouse Invite Loop'] = menu.add_feature('Warehouse Invite Loop', 'toggle', Script.Parent['Lobby Teleports'], function(f, id)
        while f.on do
            for id = 0, 31 do
                if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                    scriptevent.Send('Warehouse Invite', {player.player_id(), 0, 1, math.random(1, 22)}, id)
    ...
    Script.Feature['Lobby Force Island'] = menu.add_feature('Send to Cayo Perico v1', 'action', Script.Parent['Lobby Teleports'], function()
        for id = 0, 31 do
            if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
    ...
    Script.Feature['Lobby Force Island 2'] = menu.add_feature('Send to Cayo Perico v2', 'action_value_str', Script.Parent['Lobby Teleports'], function(f)
        for id = 0, 31 do
            if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                if f.value == 0 then
                    scriptevent.Send('Force To Island 2', {player.player_id(), 0, 0, 3, 1}, id)
        
                elseif f.value == 1 then
                    scriptevent.Send('Force To Island 2', {player.player_id(), 0, 0, 4, 1}, id)
        
                elseif f.value == 2 then
                    scriptevent.Send('Force To Island 2', {player.player_id(), 0, 0, 3, 0}, id)
        
                elseif f.value == 3 then
                    scriptevent.Send('Force To Island 2', {player.player_id(), 0, 0, 4, 0}, id)
                end
            end
        end
    end)
    Script.Feature['Lobby Force Island 2']:set_str_data({'Beach Party (Plane)', 'Beach Party (Instant)', 'Back Home (Airport)', 'Back Home (Beach)'})


    Script.Feature['Lobby Force Mission'] = menu.add_feature("Force to Mission", "action_value_str", Script.Parent['Lobby Teleports'], function(f)
        for id = 0, 31 do
            if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
    ...
    Script.Feature['Lobby Script Freeze'] = menu.add_feature('Script Freeze', 'value_str', Script.Parent['Lobby Trolling'], function(f, id)
        while f.on do
            if f.value == 0 then
                ...
                for id = 0, 31 do
                    if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                        scriptevent.Send('Force To Island 2', {player.player_id()}, id)
                    end
    ...
    Script.Feature['Lobby Start Cutscene'] = menu.add_feature('Start Casino Cutscene', 'action', Script.Parent['Lobby Trolling'], function()
        for id = 0, 31 do
            if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
    ...
    Script.Feature['Lobby Transaction Error'] = menu.add_feature('Transaction Error', 'toggle', Script.Parent['Lobby Trolling'], function(f)
        while f.on do
            for id = 0, 31 do
                if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                    scriptevent.Send('Transaction Error', {player.player_id(), 50000, 0, 1, scriptevent.MainGlobal(id), scriptevent.GlobalPair()}, id)
                end
    ...
    Script.Feature['Lobby Destroy Personal Vehicle'] = menu.add_feature('Destroy Personal Vehicle', 'action', Script.Parent['Lobby Trolling'], function()
        for id = 0, 31 do
            if scriptevent.GetPersonalVehicle(id) == 0 or not utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                goto continue
            end
    ...
    Script.Feature['Lobby CEO'] = menu.add_feature("CEO Removal", "action_value_str", Script.Parent['Lobby Griefing'], function(f)
        ...
        local name = events[f.value + 1][1]
        local args = events[f.value + 1][2]

        for id = 0, 31 do
            if utility.valid_player(id, Script.Feature['Exclude Friends'].on) and (f.value == 2 or (f.value ~= 2 and scriptevent.CEOID(id) ~= -1)) then
                scriptevent.Send(name, args, id)
            end
            
        end
    end)
	Script.Feature['Lobby CEO']:set_str_data({"Dismiss", "Terminate", "Ban"})


    Script.Feature['Lobby Set Bounty'] = menu.add_feature("Set Bounty: Input", "action_value_str", Script.Parent['Lobby Griefing'], function(f)
        if script.get_host_of_this_script() == player.player_id() then
    ...
    Script.Feature['Lobby Bounty After Death'] = menu.add_feature("Reapply Bounty after Death", "value_str", Script.Parent['Lobby Griefing'], function(f)
        if script.get_host_of_this_script() == player.player_id() then
            Notify('This doesnt work while you are Script Host!', "Error", '')
            f.on = false
            return
        ...
        while f.on do
            for id = 0, 31 do
                if utility.valid_player(id, Script.Feature['Exclude Friends'].on) and entity.is_entity_dead(get.PlayerPed(id)) then
                    ...
                    scriptevent.setBounty(id, bounty_value, f.value)
                    coroutine.yield(1000)
                end
    ...
    Script.Feature['Lobby Remove Wanted'] = menu.add_feature('Remove Wanted', 'toggle', Script.Parent['Lobby Friendly'], function(f)
        while f.on do
            for id = 0, 31 do
                if utility.valid_player(id, Script.Feature['Exclude Friends'].on) and player.get_player_wanted_level(id) > 0 then
                    scriptevent.Send('Remove Wanted', {player.player_id(), scriptevent.MainGlobal(id)}, id)
                end
        ...
        menu.add_feature(customData.custom_script_events[i].Name, 'action', Script.Parent['Lobby Custom SE'], function()
            ...
            for id = 0, 31 do
                for x = 1, #customData.custom_script_events[i].Events do
                    if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                        scriptevent.Send(customData.custom_script_events[i].Events[x].Hash, customData.custom_script_events[i].Events[x].Args, id, true)
                    end
    ...
    Script.Feature['Lobby Script Event Kick'] = menu.add_feature('Script Event Kick', 'action', Script.Parent['Lobby Malicious'], function(f)
        for id = 0, 31 do
            if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                scriptevent.kick(id)
                coroutine.yield(0)
    ...
    Script.Feature['Lobby Break'] = menu.add_feature('Break Lobby', 'action_value_str', Script.Parent['Lobby Malicious'], function(f)
        if script.get_host_of_this_script() == player.player_id() then
            Notify('This doesnt work while you are Script Host!', "Error", '')
            return
        end
        ...
        if f.value == 0 then
            for id = 0, 31 do
                if utility.valid_player(id, Script.Feature['Exclude Friends'].on) then
                    scriptevent.Send('Start CEO Mission', {player.player_id(), 0, 1}, id)
                end
        ...
        scriptevent.Send('Start CEO Mission', {player.player_id(), 0, 1}, player.player_id())
    end)
    Script.Feature['Lobby Break']:set_str_data({'v1', 'v2'})
    ...
    Script.Feature['Lobby BDSM Crash'] = menu.add_feature('BDSM Crash', 'action', Script.Parent['Lobby Malicious'], function()
    ...
    Script.Feature['Player Sector Crash'] = menu.add_feature('Player Sector Crash (Host)', 'action', Script.Parent['Lobby Malicious'], function(f)
        if network.network_is_host() then
    ...
    Script.Feature['Modder Detection Script Events'] = menu.add_feature('Modded Script Events', 'value_str', Script.Parent['local_modderdetection'], function(f)
        if hooks.modded_script == nil then
            hooks.modded_script = hook.register_script_event_hook(function(source, target, params, count)
                if utility.valid_modder(source, Script.Feature['Modder Detection Whitelist Friends'].on) and target == player.player_id() then
                    ...
                    if params[1] == scriptevent['Bounty'] and not scriptevent.DoesPlayerHaveBounty(source) and not (params[5] == 1000 or params[5] == 3000 or params[5] == 5000 or params[5] == 7000 or params[5] == 9000) then
                        guilty = true

                    elseif params[1] == scriptevent['CEO Kick'] and (scriptevent.CEOID(player.player_id()) ~= source) then
                        guilty = true

                    elseif params[1] == scriptevent['CEO Ban'] and (scriptevent.CEOID(player.player_id()) ~= source) then
                        guilty = true

                    elseif params[1] == scriptevent['CEO Money'] and (scriptevent.CEOID(player.player_id()) ~= source) then
                        guilty = true

                    elseif params[1] == scriptevent['Apartment Invite'] and (params[6] == 115 or params[6] > 128) then
                        guilty = true

                    elseif params[1] == scriptevent['Vehicle Kick'] and (get.OwnVehicle() ~= scriptevent.GetPersonalVehicle(player.player_id()) or get.OwnVehicle() ~= scriptevent.GetPersonalVehicle(source)) then
                        guilty = true
                    ...
                    elseif params[1] == scriptevent['Camera Manipulation'] and params[3] == 869796886 then
                        guilty = true

                    elseif params[1] == scriptevent['Force To Island'] then
                        guilty = true

                    elseif params[1] == scriptevent['Force To Island 2'] then
                        guilty = true

                    elseif params[1] == scriptevent['Force To Mission'] then
                        guilty = true

                    elseif params[1] == scriptevent['Force To Mission 2'] then
                        guilty = true
    ...
    Script.Feature['Modder Detection Script Events']:set_str_data({'v1', 'v2 Experimental'})
    ...
    Script.Feature['Exclude Friends'] = menu.add_feature('Exclude Friends from Lobby Events', 'toggle', Script.Parent['local_settings'], function(f)
    ...
    setup.Log('Local features successfully loaded.')
end, nil)
...
local MainThread2 = menu.create_thread(function()
    ...
    Script.Parent['Player Teleports'] = menu.add_player_feature('Teleports', 'parent', Script.Parent['Player Trolling'], nil).id

    
    Script.Feature['Player Random Apartment Invite'] = menu.add_player_feature('Random Apartment Invite', 'action', Script.Parent['Player Teleports'], function(f, id)
        scriptevent.Send('Apartment Invite', {player.player_id(), id, 1, 0, math.random(1, 114), 1, 1, 1}, id)
    end)

    
    Script.Feature['Player Apartment Invite Loop'] = menu.add_player_feature('Apartment Invite Loop', 'toggle', Script.Parent['Player Teleports'], function(f, id)
        while f.on do
            scriptevent.Send('Apartment Invite', {player.player_id(), id, 1, 0, math.random(1, 114), 1, 1, 1}, id)
            coroutine.yield(500)
        end
    ...
    Script.Feature['Player Warehouse Invite'] = menu.add_player_feature('Warehouse Invite', 'action_value_str', Script.Parent['Player Teleports'], function(f, id)
        if f.value == 22 then
            scriptevent.Send('Warehouse Invite', {player.player_id(), 0, 1, math.random(1, 22)}, id)
            return
        end
    ...
    Script.Feature['Player Warehouse Invite Loop'] = menu.add_player_feature('Warehouse Invite Loop', 'toggle', Script.Parent['Player Teleports'], function(f, id)
        while f.on do
            scriptevent.Send('Warehouse Invite', {player.player_id(), 0, 1, math.random(1, 22)}, id)
            coroutine.yield(500)
        end
    ...
    Script.Feature['Player Force Island'] = menu.add_player_feature('Send to Cayo Perico v1', 'action', Script.Parent['Player Teleports'], function(f, id)
        scriptevent.Send('Force To Island', {player.player_id(), 1}, id)
    end)

    
    Script.Feature['Player Force Island 2'] = menu.add_player_feature('Send to Cayo Perico v2', 'action_value_str', Script.Parent['Player Teleports'], function(f, id)
        if f.value == 0 then
            scriptevent.Send('Force To Island 2', {player.player_id(), 0, 0, 3, 1}, id)
    ...
    Script.Feature['Player Force Island 2']:set_str_data({'Beach Party (Plane)', 'Beach Party (Instant)', 'Back Home (Airport)', 'Back Home (Beach)'})

    
    Script.Feature['Player Force Mission'] = menu.add_player_feature("Force to Mission", "action_value_str", Script.Parent['Player Teleports'], function(f, id)
        if f.value == 0 then
	...
	Script.Feature['Player Force Mission']:set_str_data({'Severe Weather Patterns', 'Half-track Bully', 'Exit Strategy', 'Offshore Assets', 'Cover Blown', 'Mole Hunt', 'Data Breach', 'Work Dispute'})
    ...
    Script.Feature['Player Start Cutscene'] = menu.add_player_feature('Start Casino Cutscene', 'action', Script.Parent['Player Trolling'], function(f, id)
        scriptevent.Send('Casino Cutscene', {player.player_id()}, id)
    end)
    ...
    Script.Feature['Player Kill Godmode'] = menu.add_player_feature('Kill Godmode Player', 'action_value_str', Script.Parent['Player Griefing'], function(f, id)
        if id == player.player_id() then
            Notify('No point in doing this on yourself', "Error", '')
            return
        end
        ...
        ped.clear_ped_tasks_immediately(get.PlayerPed(id))
        scriptevent.Send('Camera Manipulation', {player.player_id(), 869796886, 0}, id)
        fire.add_explosion(get.PlayerCoords(id), 8, false, true, 0, blame)
    end)
    Script.Feature['Player Kill Godmode']:set_str_data({'Anonymous', 'Named'})
    ...
    Script.Feature['Player CEO'] = menu.add_player_feature("CEO Removal", "action_value_str", Script.Parent['Player Griefing'], function(f, id)
        if not f.value == 2 and scriptevent.CEOID(id) == -1 then
	...
	Script.Feature['Player CEO']:set_str_data({"Dismiss", "Terminate", "Ban"})


    Script.Feature['Player Set Bounty'] = menu.add_player_feature('Set Bounty: Input', 'action_value_str', Script.Parent['Player Griefing'], function(f, id)
        if script.get_host_of_this_script() == player.player_id() then
            Notify('This doesnt work while you are Script Host!', "Error", '')
            return
        end
    ...
    Script.Feature['Player Bounty After Death'] = menu.add_player_feature('Reapply Bounty after Death', 'value_str', Script.Parent['Player Griefing'], function(f, id)
        if script.get_host_of_this_script() == player.player_id() then
            Notify('This doesnt work while you are Script Host!', "Error", '')
            f.on = false
            return
    ...
    Script.Feature['Player Asteroid'] = menu.add_player_feature('Falling Asteroids', 'action_value_str', Script.Parent['Player Griefing'], function(f, id)
        if f.value == 0 then
            for i = 1, 25 do
                local pos = get.PlayerCoords(id)
    ...
    Script.Feature['Player Asteroid']:set_str_data({'Start', 'Delete'})
    ...
    Script.Feature['Player Spam Script Events'] = menu.add_player_feature('Spam Script Events', 'value_str', Script.Parent['Player Griefing'], function(f, id)
        ...
        for i = 0, 31 do
            if i ~= id and player.is_player_valid(i) and Script.Feature['Player Spam Script Events'].on[i] then
                Notify('Feature is already on for another Player.', "Error", '')
                f.on = false
                return
            end
        ...
        while f.on do
            ...
            scriptevent.Send('Force To Island', {player.player_id(), 1}, id)
            scriptevent.Send('Force To Island 2', {player.player_id(), 0, 0, math.random(3, 4), math.random(0, 1)}, id)
            coroutine.yield(0)
            scriptevent.Send('Vehicle Kick', {player.player_id(), 4294967295, 4294967295, 4294967295}, id)
            scriptevent.Send('CEO Kick', {player.player_id(), 1, math.random(5, 6)}, id)
    ...
    for i = 1, #miscdata.ceomoney do
        Script.Feature['Player CEO Loop ' .. i] = menu.add_player_feature(miscdata.ceomoney[i][1] .. ' (ms)', 'value_i', Script.Parent['Player CEO Money'], function(f, id)
            ...
            if scriptevent.CEOID(id) == -1 or scriptevent.CEOID(id) == id then
                Notify('Player is not an associate.', "Error", '')
                f.on = false
                return
            end
            ...
            while f.on do
                if scriptevent.CEOID(id) ~= -1 and not scriptevent.CEOID(id) == id then
                    scriptevent.Send('CEO Money', {player.player_id(), miscdata.ceomoney[i][2], miscdata.ceomoney[i][3], miscdata.ceomoney[i][4], scriptevent.MainGlobal(id), scriptevent.GlobalPair()}, id)
                end
    ...
    Script.Feature['Player Off The Radar'] = menu.add_player_feature('Off The Radar', 'action', Script.Parent['Player Friendly'], function(f, id)
        if not scriptevent.IsPlayerOTR(id) then
            scriptevent.Send('Off The Radar', {player.player_id(), utils.time() - 60, utils.time(), 1, 1, scriptevent.MainGlobal(id)}, id)
        end
    end)

    Script.Feature['Player Off The Radar Loop'] = menu.add_player_feature('Off The Radar', 'toggle', Script.Parent['Player Friendly'], function(f, id)
        while f.on do
            if not scriptevent.IsPlayerOTR(id) then
                scriptevent.Send('Off The Radar', {player.player_id(), utils.time() - 60, utils.time(), 1, 1, scriptevent.MainGlobal(id)}, id)
            end
    ...
    Script.Feature['Player Spam Custom SMS'] = menu.add_player_feature('Spam SMS: Input', 'toggle', Script.Parent['Player SMS Sender'], function(f, id)
        ...
        while f.on do
            ...
            coroutine.yield(Script.Feature['Player SMS Delay'].value[id])
        end
    end)
    ...
    Script.Feature['Player Script Event Kick'] = menu.add_player_feature('Script Event Kick', 'action', Script.Parent['Player Malicious'], function(f, id)
        if id == player.player_id() then
            Notify('No point in doing this on yourself', "Error", '')
            return
        end
    ...
    Script.Feature['World Object Crash'] = menu.add_player_feature('World Object Crash', 'action_value_str', Script.Parent['Player Malicious'], function(f, id)
        ...
        local worldhashes = {
            ...
            3301528862, 3303982422, 3476535839, 3726116795, -1231365640
        }

        if f.value == 0 then
            local hash = worldhashes[tonumber(#worldhashes)]
    ...
    Script.Feature['Invalid Entity Crash'] = menu.add_player_feature('Invalid Entity Crash', 'action_value_str', Script.Parent['Player Malicious'], function(f, id)
        ...
        if f.value ~= 2 and Position:magnitude(Coords) < 1000 then
            Notify('You are too close to the Target.', "Error", '')
            return
        end
    ...
    Script.Feature['Player AIO Crash'] = menu.add_player_feature('AIO', 'action', Script.Parent['Player Malicious'], function(f, id)
        ...
        Notify('AIO Crash starting in 5 seconds.\nDont go near the Target.', "Neutral", '')
        coroutine.yield(5000)
        ...
        Notify('AIO Crash sent, attemping cleanup in 5 seconds.', "Success", '')
        coroutine.yield(5000)
    ...
    setup.Log('Player features successfully loaded.')
end, nil)
-- End of Player Features

menu.create_thread(function()
    while not menu.has_thread_finished(MainThread1) and not menu.has_thread_finished(MainThread2) do
        coroutine.yield(0)
    end
