...
local wpn = {0x958A4A8F, 0x6D544C99, 0x99B507EA}
local wpnsel
local runs = 0
local pid
local pname
...
--	killer_del.threaded = true
    ...
    local boobswpnset = menu.add_feature("set Weapon 1:Bat 2:Railgun 3:knife", "action_value_i", moist.id, set_boobs_weapon)
    boobswpnset.max_i = #wpn
    boobswpnset.min_i = 1
    boobswpnset.value_i = 1

    local boobs_para = menu.add_feature("Send 1x Topless woman Parachuting", "action", boob.id, send_boobs_para)

    -- local boobretask = menu.add_feature("task boobs to Attack", "action", boob.id, boobs_task)
    -- boobretask.threaded = false

    local boobs_para01 = menu.add_feature("Send 1-20x boobs(s)", "action_value_i", boob.id, send_boobs)
    ...
    local lester_para = menu.add_feature("Send 1x Lester Parachuting", "action", lesatk.id, send_lester_para)

    -- local retask = menu.add_feature("task Lester to Attack", "action", lesatk.id, lester_task)
    -- retask.threaded = false

    local lester_para01 = menu.add_feature("Send 1-20x Lesters", "action_value_i", lesatk.id, send_lester)
