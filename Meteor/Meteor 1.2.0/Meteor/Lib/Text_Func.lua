...
function text_func.split_string(split_string, splitter, mode)
    local parts = {}
    if mode == 2 then
        for match in split_string:gmatch("[^" .. splitter .. "]+")do
            table.insert(parts, match)
        end
    else
        for match in (split_string .. splitter):gmatch("(.-)" .. splitter) do
            table.insert(parts, match)
        end
    end
    return parts
...
function text_func.to_boolean(Any)
    if Any then
        if tonumber(Any) == 0 then
            return false
        else
            return true
        end
