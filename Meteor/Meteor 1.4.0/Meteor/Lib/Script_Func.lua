...
function script_func.is_script_entity(Entity)
...
function script_func.get_script_owner_of_entity(Entity)
	if Entity and entity.is_an_entity(Entity) then
		if natives.GET_ENTITY_SCRIPT(Entity, 0):__tostring(true) ~= nil then
			return natives.NETWORK_GET_HOST_OF_SCRIPT(natives.GET_ENTITY_SCRIPT(Entity, 0):__tostring(true), -1, 0):__tointeger()
		end
		if text_func.to_boolean(natives.DOES_ENTITY_BELONG_TO_THIS_SCRIPT(Entity, true):__tointeger()) then
			return script.get_host_of_this_script()
		end
		if ped.is_ped_a_player(vehicle.get_ped_in_vehicle_seat(Entity, -1)) and not network.has_control_of_entity(Entity) then
			return player.get_player_from_ped(vehicle.get_ped_in_vehicle_seat(Entity, -1))
		end
