...
function utilities.max_vehicle(vehicle_)
    if vehicle_ then
		...
		for i = 1, #upgrades do
			local mod = vehicle.get_num_vehicle_mods(vehicle_, upgrades[i]) - 1
			vehicle.set_vehicle_mod(vehicle_, upgrades[i], mod, true)
		end
		vehicle.toggle_vehicle_mod(vehicle_, 18, true)
		vehicle.set_vehicle_bulletproof_tires(vehicle_, true)
	end
end
...
function utilities.request_control(Entity)
	...
	while entity.is_an_entity(Entity) and not network.has_control_of_entity(Entity) and time > utils.time_ms() do
		system.yield(0)
		network.request_control_of_entity(Entity)
	end
...
function utilities.request_control_fast_silent(Entity)
