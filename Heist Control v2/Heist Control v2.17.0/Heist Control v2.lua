...
--
local Heist_Control = menu.add_feature("» Heist Control v2.17.0", "parent", 0, function()
end)
...
local PED_MNGR = menu.add_feature("» Ped Weapons [NPC]", "parent", TOOLS.id)
...
local CONTRACT_MANAGER = menu.add_feature("» VIP Contract : Dr. Dre", "parent", TH_CONTRACT.id, function()
    menu.notify("You may need to log out of\n\tF.Clinton & Partner\n\t... to update properly")
end)
...
menu.add_feature("» Modify Final Contract Payout (2 Millions | You only)", "toggle", TH_CONTRACT.id, function(TH_)
    while TH_.on do
        script.set_global_i(262145+31373,2000000)
...
menu.add_feature("» Set Fire extinguisher as weapon", "toggle", PED_MNGR.id, function(__RE)
    while __RE.on do
    for k, PedIA in pairs(ped.get_all_peds()) do
        if not ped.is_ped_a_player(PedIA) then
        ped.set_ped_combat_movement(PedIA, 0)
        ped.set_ped_can_switch_weapons(PedIA,false)
        weapon.give_delayed_weapon_to_ped(PedIA, 0x060EC506, 1000, true)
        weapon.set_ped_ammo(PedIA, 0x060EC506, 50)
    if not __RE.on then return end end end
    system.yield(2000)
    end
end)
