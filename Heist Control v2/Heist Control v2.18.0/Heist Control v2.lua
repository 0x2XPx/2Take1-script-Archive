...
if menu.is_trusted_mode_enabled() then
    menu.notify(": Planning to repeat the same Heist?\n\tWait 16 minutes to repeat it again.\n\n: It is recommended to create a profile for Heists with low protection.\n\n: Keep Trusted Mode activated while using Heist Control.", "Heist Control - Tips", 7, 0x64FA7878)
    menu.notify("Dev: jhowkNx\n\nCurrent version: 2.18.0\nUpdated on 31/Dec/2021", "Welcome to Heist Control", 5, 0x64DC78F0)
    end
--
...
--
local Heist_Control = menu.add_feature("» Heist Control v2.18.0", "parent", 0, function()
end)
...
menu.add_feature("Ocean : Escape", "action", TELEPORT_QL.id, function()
...
local SET_NORMAL = {
    ...
    script.set_global_i(262145 + 28454, 1410065408) --Art
...
local DD_H_ACT1 = {
    ...
    menu.add_feature("» ACT I : The Data Breaches", "action", DOOMS_PRESETS.id, function()
...
local DD_H_ACT2 = {
    ...
    menu.add_feature("» ACT II : The Bogdan Problem", "action", DOOMS_PRESETS.id, function()
...
local DD_H_ACT3 = {
    ...
    menu.add_feature("» ACT III : The Doomsday Scenario", "action", DOOMS_PRESETS.id, function()
...
menu.add_feature("» Modify ACT I Payment [$2.5 Millions]", "toggle", DDHEIST_PLYR_MANAGER.id, function(act1)
    menu.notify("ACT I Compatible\n\nRecommended difficulty: Hard\n\nIn-game percentage may seem strange, but it's ok!\n\nKeep the function active until the end of the Heist", "", 6, 0x6414F0FF)
    menu.notify("Estimated payout for each player:\n $2.550,000", "", 3, 0x50F0FF14)
    while act1.on do
    ...
    if not act1.on then return end
    system.wait(0)
end
end)

menu.add_feature("» Modify ACT II Payment [$2.5 Millions]", "toggle", DDHEIST_PLYR_MANAGER.id, function(act2)
    menu.notify("ACT II Compatible\n\nRecommended difficulty: Hard\n\nIn-game percentage may seem strange, but it's ok!\n\nKeep the function active until the end of the Heist", "", 6, 0x6414F0FF)
    menu.notify("Estimated payout for each player:\n $2.550,000", "", 3, 0x50F0FF14)
    while act2.on do
    ...
    if not act2.on then return end
    system.wait(0)
end

end)

menu.add_feature("» Modify ACT III Payment [$2.5 Millions]", "toggle", DDHEIST_PLYR_MANAGER.id, function(act3)
    menu.notify("ACT III Compatible\n\nRecommended difficulty: Hard\n\nIn-game percentage may seem strange, but it's ok!\n\nKeep the function active until the end of the Heist", "", 6, 0x6414F0FF)
    menu.notify("Estimated payout for each player:\n $2.550,000", "", 3, 0x50F0FF14)
    while act3.on do
    ...
    if not act3.on then return end
    system.wait(0)
end
end)
...
local CONTRACT_MANAGER = menu.add_feature("» VIP Contract : Dr. Dre", "parent", TH_CONTRACT.id, function()
    menu.notify("You must log off to update correctly")
end)
...
menu.add_feature("» Modify Final Contract Payout (2.4 Millions | You only)", "toggle", TH_CONTRACT.id, function(TH_)
    while TH_.on do
...
menu.add_feature("» [EVENT x1.5] Modify Final Contract Payout (2.4 Millions | You only)", "toggle", TH_CONTRACT.id, function(TH_0)
    menu.notify("Only use this option when there is a Double Payment event.", "", 4, 0x50F05014)
    while TH_0.on do
...
menu.add_feature("» Remove Security Missions Cooldown", "toggle", TH_CONTRACT.id, function(CNT_CL0)
    while CNT_CL0.on do
...
menu.add_feature("» Unlock Dripfed Vehicles (Unreleased)", "toggle", TH_CONTRACT.id, function(_T)
...
menu.add_feature("» Modify Vehicle Top Speed", "action", TOOLS.id, function()
    ...
    local r, s = input.get("Enter new Torque", 65, 3)
...
menu.add_feature("» Remove all ped weapons", "toggle", TOOLS.id, function(_RE)
    while _RE.on do
    for k, PedIA in pairs(ped.get_all_peds()) do
        ...
        if not _RE.on then return end end end
    system.yield(1500)
    end
end)

menu.add_feature("» Allow Missions/Preps/Jobs on non public lobbies", "toggle", TOOLS.id, function(_mi)
    while _mi.on do
    script.set_global_i(2714627+744, 0) -- NETWORK::NETWORK_SESSION_GET_PRIVATE_SLOTS
    if not _mi.on then return end
    system.wait(0)
    end
    menu.notify("Stuffs like sell/buy/preps/missions will be able to do now")
end)
