...
if menu.is_trusted_mode_enabled() then
    ...
    menu.notify("Dev: jhowkNx\n\nCurrent version: v2.22.1\nUpdated on 19/June/2022", "Welcome to Heist Control", 7,0x50006414)
...
--
Heist_Control = '» Heist Control v1.22.1'
--
local Heist_Control = menu.add_feature("» Heist Control v2.22.1", "parent", 0, function()
end)
--
...
menu.add_feature("» Move wallet money to my bank", "action", MyWallet.id, function()
	local WA_CASH = native.call(0xA40F9C2623F6A8B5, charID):__tointeger()
	local BK_CASH = native.call(0x76EF28DA05EA395A):__tointeger()

	if WA_CASH > 0 then
		native.call(0xC2F7FE5309181C7D, charID, WA_CASH):__tostring()
...
menu.add_feature("» Move bank money to my wallet", "action", MyWallet.id, function()
	local WA_CASH = native.call(0xA40F9C2623F6A8B5, charID):__tonumber()
	local BK_CASH = native.call(0x76EF28DA05EA395A):__tointeger()

	if BK_CASH > 0 then
		native.call(0xD47A2C1BA117471D, charID, BK_CASH):__tostring()
        audio.play_sound_from_coord(-1, "ROBBERY_MONEY_TOTAL", player.get_player_coords(player.player_id()), "HUD_FRONTEND_CUSTOM_SOUNDSET", false, 0, true)
    end
end)
