...
function randomizeHeadBlend(mp_ped)
    ...
    if (entity.get_entity_model_hash(mp_ped) == 2627665880) then
        ped.set_ped_head_blend_data(mp_ped, hb_randomizer["fem_face_shapes1"], hb_randomizer["fem_face_shapes2"], 0, math.random(0, 45), math.random(0, 45), 0, math.random(), math.random(), 0)
        local xyz = math.random(0, 80)
        if (xyz ~= 24) then
            ped.set_ped_component_variation(mp_ped, 2, xyz, 0, 0) --hair 
        end
    else
        ped.set_ped_head_blend_data(mp_ped, hb_randomizer["male_face_shapes1"], hb_randomizer["male_face_shapes1"], 0, math.random(0, 45), math.random(0, 45), 0, math.random(), math.random(), 0) -- base headblend
