...
fun_menu_pages["character_looks"] = menu.add_feature("Character Looks", "parent", fun_menu_pages["main_menu"].id)
...
local fun_menu_ver = "Fun Menu v0.5.0"
...
local silent_fireworks = menu.add_feature("Silent Fireworks?", "toggle", fun_menu_pages["festive"].id, function(feat_toggle)
    ...
    if (feat_toggle.on) then
        ...
        for i = 1, angry_vehicles_count.value do
            if (#spawned_planes < angry_vehicles_count.value) then
                ...
                ai.task_vehicle_drive_to_coord(pilot, plane, player.get_player_coords(player.player_id()) + v3(0, 0, math.random(0, 50)), 100.0, -1, gameplay.get_hash_key(planes[random_plane]), 0, 0.0, 1.0)
                --ai.task_vehicle_follow(pilot, plane, player.get_player_ped(player.player_id()), 30, 786859, 30)

                x = x + 1
                menu.notify(x .. " planes spawned")
            end
        end
    end
    ...
    if (feat_toggle.on) then
        ...
        for i = 1, angry_vehicles_count.value do
            if (#spawned_helicopters < angry_vehicles_count.value) then
                ...
                x = x + 1
                menu.notify(x .. " helicopters spawned")
            end
        end
    end
...
menu.add_feature("Crazy Vehicles", "value_str", fun_menu_pages["vehicle_stuff"].id, function(feat_toggle_val)
    ...
    while (feat_toggle_val.on) do
        ...
        for i = 1, #vehicles do
            ...
            if (feat_toggle_val.value == 0) then
                vehicle.modify_vehicle_top_speed(vehicles[i], 500.0)
...
menu.add_feature("Anti-Gravity Vehicles", "value_str", fun_menu_pages["vehicle_stuff"].id, function(feat_toggle_val)
    local vehicles
    while (feat_toggle_val.on) do
        vehicles = vehicle.get_all_vehicles()
            ...
            while (feat_toggle_val.on) do
                for i = 1, #vehicles do
                    ...
                    elseif (feat_toggle_val.value == 1) then
                        ...
                        entity.set_entity_gravity(vehicles[i], false)
                        entity.set_entity_velocity(vehicles[i], v3(0, 0, 1))
                        if (not has_rotated[vehicles[i]]) then
                            entity.set_entity_rotation(vehicles[i], v3(math.random(-180, 180), math.random(-180, 180), math.random(-180, 180)))
                            has_rotated[vehicles[i]] = true
                        end
                    end
                end
                system.wait(500)
...
menu.add_feature("Make Self Barefoot", "action", fun_menu_pages["character_looks"].id, function(feat)
...
menu.add_feature("Make Self Shirtless", "action", fun_menu_pages["character_looks"].id, function(feat)
...
menu.add_feature("Make Self an Old Codger", "action", fun_menu_pages["character_looks"].id, function(feat)
    local codger_colors = {
...
menu.add_feature("Random Face Features", "action_value_str", fun_menu_pages["character_looks"].id, function(feat_val)
...
menu.add_feature("Random Headblend", "action_value_str", fun_menu_pages["character_looks"].id, function(feat_val)
    if (feat_val.value == 0) then
...
menu.add_feature("Random Skin Tone", "action", fun_menu_pages["character_looks"].id, function(feat)
    randomizeSkinTone(player.get_player_ped(player.player_id()))
end)

menu.add_feature("Wipe Headblend", "action", fun_menu_pages["character_looks"].id, function(feat)
    wipeHeadBlend(player.get_player_ped(player.player_id()))
end)

menu.add_feature("Random Funky Hair Color", "action", fun_menu_pages["character_looks"].id, function(feat_val)
...
menu.add_feature("Hairy Disco", "toggle", fun_menu_pages["character_looks"].id, function(feat_toggle)
    while (feat_toggle.on) do
...
menu.add_feature("Spawn Clone Follower(s)", "toggle", fun_menu_pages["follower_menu"].id, function(feat_toggle)
    ...
    for i = 1, follower_count.value do
        follower_peds[i] = ped.create_ped(1, MP_ped, player.get_player_coords(player.player_id()) + v3(math.random(-2, 2), math.random(-2, 2), 0), math.random(0, 360), true, false)
    ...
    while feat_toggle.on do
        for i = 1, follower_count.value do
            local entity_pos = entity.get_entity_coords(follower_peds[i])
...
menu.add_feature("Spawn Bird:", "action_value_str", fun_menu_pages["npc_stuff"].id, function(feat_value)
    ...
    local bird = ped.create_ped(5, gameplay.get_hash_key(BirdModels[feat_value.value + 1]), player.get_player_coords(player.player_id()) + v3(math.random(-2, 2), math.random(-2, 2), 0), 0, true, false)
...
menu.add_feature("Spawn Wild Animal:", "action_value_str", fun_menu_pages["npc_stuff"].id, function(feat_value)
    ...
    local wild_animal = ped.create_ped(5, gameplay.get_hash_key(WildAnimalModels[feat_value.value + 1]), player.get_player_coords(player.player_id()) + v3(math.random(-2, 2), math.random(-2, 2), 0), 0, true, false)
...
end):set_str_data(WildAnimalNames) --task_wander_standard(Ped ped, float unk0, bool unk1)
...
menu.add_feature("Clumsiness", "value_str", fun_menu_pages["miscellaneous"].id, function(feat_toggle_val)
...
menu.add_feature("Trip on Something", "action", fun_menu_pages["miscellaneous"].id, function(feat)
    ped.set_ped_to_ragdoll(player.get_player_ped(player.player_id()), math.random(1, 25) * 100, math.random(1, 25) * 100, math.random(-25, 25))
end)
...
menu.add_feature("Play Dead", "value_str", fun_menu_pages["miscellaneous"].id, function(feat_toggle_val)
...
menu.add_feature("Hands Up Animation", "toggle", fun_menu_pages["miscellaneous"].id, function(feat_toggle)
    requestAnimDictAndSet("random@mugging3", "handsup_standing_base")


    if (feat_toggle.on) then
        fun_menu_threads["animation"] = menu.create_thread(function()
            ai.task_play_anim(player.get_player_ped(player.player_id()), "random@mugging3", "handsup_standing_base", 8.0, 0, -1, 9, 0, false, false, false)
        end, nil)
    end

    if (not feat_toggle.on) then
        ped.clear_ped_tasks_immediately(player.get_player_ped(player.player_id()))
        unloadAnim("random@mugging3", "handsup_standing_base")
        menu.delete_thread(fun_menu_threads["animation"])
    end
end)
...
--[[
    menu.add_feature("Telescope", "toggle", fun_menu_pages["miscellaneous"].id, function(feat_toggle)
    local telescope = graphics.request_scaleform_movie("REMOTE_SNIPER_HUD")
    while (feat_toggle.on) do
        show_sniper_scope(telescope, 0, math.random(1, 100), false)
        system.wait(0)
    end

    if (not feat_toggle.on) then
        graphics.set_scaleform_movie_as_no_longer_needed(telescope)
    end
end)

menu.add_feature("Buttons", "toggle", fun_menu_pages["miscellaneous"].id, function(feat_toggle)
    local buttons = graphics.request_scaleform_movie("INSTRUCTIONAL_BUTTONS")
    while (feat_toggle.on) do
        graphics.begin_scaleform_movie_method(buttons, "SET_DISPLAY_CONFIG")
        ...
        graphics.begin_scaleform_movie_method(buttons, "DRAW_INSTRUCTIONAL_BUTTONS")
        graphics.draw_scaleform_movie_fullscreen(buttons, 255, 255, 255, 255, 0)
        graphics.end_scaleform_movie_method(buttons)
        system.wait()
    end

    if (not feat_toggle.on) then
        graphics.set_scaleform_movie_as_no_longer_needed(buttons)
    end
end)

local zoom = menu.add_feature("View Orbital Map", "value_f", fun_menu_pages["miscellaneous"].id, function(feat_toggle)
    local orbital_map = graphics.request_scaleform_movie("ORBITAL_CANNON_MAP")
    while (feat_toggle.on) do
        graphics.draw_scaleform_movie_fullscreen(orbital_map, 255, 255, 255, 255, 0)
        graphics.begin_scaleform_movie_method(orbital_map, "ZOOM_TO")
        graphics.scaleform_movie_method_add_param_float(feat_toggle.value)

        graphics.begin_scaleform_movie_method(orbital_map, "MOVE_TO")
        graphics.scaleform_movie_method_add_param_int(100)
        graphics.scaleform_movie_method_add_param_int(100)

        graphics.end_scaleform_movie_method(orbital_map)
        system.wait(0)
    end

    if (not feat_toggle.on) then
        graphics.set_scaleform_movie_as_no_longer_needed(orbital_map)
    end
end)
zoom.min = 0
zoom.max = 2
...
local state_test = menu.add_feature("View Orbital Cam", "value_i", fun_menu_pages["miscellaneous"].id, function(feat_toggle)
    local orbital_cam = graphics.request_scaleform_movie("ORBITAL_CANNON_CAM")
    while (feat_toggle.on) do
        graphics.begin_scaleform_movie_method(orbital_cam, "SET_STATE")
        graphics.scaleform_movie_method_add_param_int(feat_toggle.value)

        graphics.begin_scaleform_movie_method(orbital_cam, "SET_MENU_TITLE")
        graphics.scaleform_movie_method_add_param_texture_name_string("fewqfwqefqewqweffewqfeqw")

        graphics.begin_scaleform_movie_method(orbital_cam, "SET_ZOOM_LEVEL")
        graphics.scaleform_movie_method_add_param_int(0)

        graphics.begin_scaleform_movie_method(orbital_cam, "SET_CHARGING_LEVEL")
        graphics.scaleform_movie_method_add_param_float(1.0)

        graphics.begin_scaleform_movie_method(orbital_cam, "SET_COOLDOWN_LEVEL")
        graphics.scaleform_movie_method_add_param_float(1.0)
        graphics.draw_scaleform_movie_fullscreen(orbital_cam, 255, 255, 255, 255, 0)

        graphics.end_scaleform_movie_method(orbital_cam)
        system.wait(0)
    end

    if (not feat_toggle.on) then
        graphics.set_scaleform_movie_as_no_longer_needed(orbital_cam)
    end
end)
state_test.min = -10
state_test.max = 10
...
menu.add_feature("Yacht Text", "toggle", fun_menu_pages["miscellaneous"].id, function(feat_toggle)
    local yacht_name = graphics.request_scaleform_movie("YACHT_NAME")
    while (feat_toggle.on) do
        show_yacht_name(yacht_name, "eee", true, "efff")
        system.wait(0)
    end

    if (not feat_toggle.on) then
        graphics.set_scaleform_movie_as_no_longer_needed(yacht_name)
    end
end)

menu.add_feature("Psych Report", "toggle", fun_menu_pages["miscellaneous"].id, function(feat_toggle)
    local psych_report = graphics.request_scaleform_movie("PSYCHOLOGY_REPORT")
    while (feat_toggle.on) do
        graphics.draw_scaleform_movie_fullscreen(psych_report, 255, 255, 255, 255, 0)
        graphics.begin_scaleform_movie_method(psych_report, "SET_PLAYER_NAME")
        graphics.scaleform_movie_method_add_param_texture_name_string("e")
        graphics.scaleform_movie_method_add_param_texture_name_string("f")
        graphics.scaleform_movie_method_add_param_texture_name_string("g")
        graphics.end_scaleform_movie_method(psych_report)
        system.wait(0)
    end

    if (not feat_toggle.on) then
        graphics.set_scaleform_movie_as_no_longer_needed(psych_report)
    end
end)
]]