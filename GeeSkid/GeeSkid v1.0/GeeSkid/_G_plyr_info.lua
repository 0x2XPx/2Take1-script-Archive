...
function _GF_rmv_wntd_lvl(_pid)
		...
		for i=1,32 do
			if _GF_valid_pid(i-1) then
				if _G_plyrs_overlay_main.dist_table[i] == nil or _G_plyrs_overlay_main.dist_table[i][1] == 10000 then
					_G_plyrs_overlay_main.dist_table[i]={_GF_dist_me_pid(i-1),player.get_player_coords(i-1)}
				elseif i-1 == player.player_id() then
					_G_plyrs_overlay_main.dist_table[i]={0,player.get_player_coords(i-1)}
				...
				_G_plyrs_overlay_main.dist_table[i]={10000,v3(0,0,0)}
				system.yield(3)
			end
		end
		check = false
