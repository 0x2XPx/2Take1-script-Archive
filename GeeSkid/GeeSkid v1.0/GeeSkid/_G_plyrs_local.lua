...
function _G_plyrs_local_do(parent)
	...
	_GT_sssn_stf.spwn_prnt = menu.add_feature("Spawn Vehicles", "parent", parent.id)
		...
		menu.add_feature("Quick vehicle model search", "action", _GT_sssn_stf.spwn_prnt.id, function(f)
			_GF_veh_spawn_quick(_GT_sssn_stf.quick_spwn,"Vehicle Spawn - Session",true)
		end)

		_GT_sssn_stf.action_spwn = {}
		for i=1,#_GT_class_str_list do
			_GT_sssn_stf.action_spwn[#_GT_sssn_stf.action_spwn+1] = menu.add_feature(_GT_class_str_list[i], "parent", _GT_sssn_stf.spwn_prnt.id)
			for ii=1,#_GT_all_veh_info do
				if _GT_all_veh_info[ii][6] == _GT_class_str_list[i] then
					menu.add_feature(_GT_all_veh_info[ii][3], "action", _GT_sssn_stf.action_spwn[i].id,function(f,pid)
	...
	_GT_sssn_stf.list_spwn_feat=menu.add_feature("Search for vehicles", "action_value_str", _GT_sssn_stf.spwn_srch_prnt.id, function(f)
		...
		if status == 0 then
			_GT_sssn_stf.list_spwn_temp = {}
			_GF_veh_search_do(str,f.value,_GT_sssn_stf.list_spwn_temp)
			...
			for pid in _GF_all_players_but_me() do
				if _GF_session_player_do(pid) then
					if _GF_is_pid_intrr(pid) then
						...
						veh = _GF_spawn_veh_at_pid_frnt(pid,_hash,true)
					end
				end
			end
			if int_count > 0 then
