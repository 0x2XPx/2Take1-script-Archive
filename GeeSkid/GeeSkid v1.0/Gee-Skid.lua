...
function _GT_veh_esp_table.check_for_plyr(_veh,_bool)
	...
	if _bool then
		if _pidT ~= nil then 
			return r,g,b,(_pidT-1),true,count,_GT_plyr_info.is_frnd[_pidT],(_pidT-1 == player.player_id())
		end
	elseif count > 0 then
		return true
	end
	for i=1,#_GT_plyr_info.veh do
		if (_GT_plyr_info.veh[i] == _veh) then
			if _bool then
				...
				return r,g,b,(i-1),false,0,_GT_plyr_info.is_frnd[i],(i-1 == player.player_id())
			end
			return true
		end
	end
	if _bool and _GT_veh_esp_table.veh_table[_veh+1][12] ~= false then
		...
		r,g,b = _G_plyrs_overlay_main.plyr_rgb(_pidT,nil)
		return r,g,b,(_pidT-1),false,count,_GT_plyr_info.is_frnd[_pidT],(_pidT-1 == player.player_id())
	end
	if _bool then
		return 255,255,255, -1,false,0,false,false
	end
		...
		for i=1,#all_veh do
			...
			if _GT_veh_esp_table.veh_table[veh+1] ~= nil and entity.is_an_entity(veh) and _GT_veh_esp_table.veh_table[veh+1][2] < _GT_veh_esp_table.dist.value and (_GT_veh_esp_table.veh_table[veh+1][3] or _GT_veh_esp_table.veh_table[veh+1][4]) then
				...
				if bool then
					...
					if show_pssngr then
						if _GT_veh_esp_table.veh_table[veh+1][14] == 0 then
							pssngr_count="<Empty>"
						elseif _GT_veh_esp_table.veh_table[veh+1][14] == 1 then
							pssngr_count="<".._GT_veh_esp_table.veh_table[veh+1][14].." player>"
						else
...
_GT_veh_esp_table.feat_record=menu.add_feature("Vehicle ESP record hidden", "toggle", _G_veh_esp_option.id, function(f)
	local all_veh,veh,r,g,b,pid,class,hash,prsnl,in_veh,pssngrs,frnd,is_me
	...
	local function record_veh()
		system.yield(5)
		r,g,b,pid,in_veh,pssngrs,frnd,is_me = _GT_veh_esp_table.check_for_plyr(veh,true)
	...
	while f.on do
		...
		for i=1,#all_veh do
			...
			if _GT_veh_esp_table.good_esp_veh(veh) and show_my_veh() then
				...
				if _GT_veh_esp_table.png_true[hash] then -- if i have an image for that vehicle
					record_veh()
					...
					_GT_veh_esp_table.veh_table[veh+1][11] = (_GF_model_name(veh) == "Buzzard Attack Chopper" and _GT_veh_esp_table.veh_table[veh+1][13])
				elseif (_GT_veh_esp_table.png_true[class] and _GT_veh_esp_table.check_for_plyr(veh,false)) then
					record_veh()
					...
					_GT_veh_esp_table.veh_table[veh+1][11] = (class == "Helicopters" and _GT_veh_esp_table.veh_table[veh+1][13])
				else
					_GT_veh_esp_table.veh_table_default(veh+1)
				end
			else
...
_GF_set_feat_i_f(_GT_veh_esp_table.text_size,.25,2,.05,1)

_GT_veh_esp_table.text_space = menu.add_feature("Spacing","action_value_f",_GT_veh_esp_table.text_parent.id)
_GF_set_feat_i_f(_GT_veh_esp_table.text_space,.25,2,.05,1)
...
-- _GT_veh_esp_table.x_offset = menu.add_feature("Manual X Offset","autoaction_value_f",_G_veh_esp_option.id) -- need to do math to account for difference between scriptdraw and ui
-- _GT_veh_esp_table.x_offset.min = -1.00
-- _GT_veh_esp_table.x_offset.max = 1.00
-- _GT_veh_esp_table.x_offset.mod = 0.001
-- _GT_veh_esp_table.x_offset.value = 0
		...
		elseif _GT_gta_map.show_map > 0 then
			...
			if _GT_gta_map.show_map == 4 then
				if _GT_gta_map.cayo then
					scriptdraw.draw_sprite(_GT_gta_map.cayo_3d,v2((_map_x+(.3732*_map_size)),(_map_y+(-1.0396*_map_size))),_map_size*.1799,0,_map_alpha)
				end
			...
			if _GT_gta_map.cayo then
				...
				scriptdraw.draw_sprite(_GT_gta_map.cayo_3d,v2((_map_x+(0.4357*_map_size)),(_map_y+(-1.1664*_map_size))),_map_size*0.1534,0,_map_alpha)
			end
		end
...
_GF_set_feat_i_f(_GT_gta_map.head1_alpha_feat,.25,1,.05,1)
...
_GF_set_feat_i_f(_GT_gta_map.map1_alpha_feat,.1,1,.05,.5)
...
_GF_set_feat_i_f(_GT_gta_map.plyr1_alpha_feat,.1,1,.05,1)
...
_GF_set_feat_i_f(_GT_gta_map.head2_alpha_feat,.25,1,.05,1)
...
_GF_set_feat_i_f(_GT_gta_map.map2_alpha_feat,.1,1,.05,.69)
...
_GF_set_feat_i_f(_GT_gta_map.plyr2_alpha_feat,.1,1,.05,1)
...
_GT_gta_map.fuckup_parent=menu.add_feature("Monitor/ratio adjustments","parent",_G_GTA_map_parent.id,function(f)
	...
	menu.notify("Screen resolution "..screen_x.."/"..screen_y.."\nRatio "..math.floor(screen_x/div)..":"..math.floor(screen_y/div).."\nAdjustments should only be required for ratios other than 16:9",_G_GeeVer,5)
end)
 
_GT_gta_map.full_math_x = menu.add_feature("Mark current map","action",_GT_gta_map.fuckup_parent.id,function(f)
...
---------------------------------------------------------------------------------------------------------------
_GT_gta_map.fuckup_all=menu.add_feature("Set for ALL maps","parent",_GT_gta_map.fuckup_parent.id)

_GT_gta_map.all_math_x = menu.add_feature("X offset","action_value_f",_GT_gta_map.fuckup_all.id,function(f)
...
_GT_gta_map.all_math_y = menu.add_feature("Y offset","action_value_f",_GT_gta_map.fuckup_all.id,function(f)
...
---------------------------------------------------------------------------------------------------------------
_GT_gta_map.fuckup_full=menu.add_feature("Full map","parent",_GT_gta_map.fuckup_parent.id)
...
---------------------------------------------------------------------------------------------------------------
_GT_gta_map.fuckup_qrtr=menu.add_feature("Quarter maps","parent",_GT_gta_map.fuckup_parent.id)
...
---------------------------------------------------------------------------------------------------------------
_GT_gta_map.fuckup_cntr=menu.add_feature("Center zoom maps","parent",_GT_gta_map.fuckup_parent.id)
...
---------------------------------------------------------------------------------------------------------------
_GT_gta_map.fuckup_othr=menu.add_feature("Other zoom maps","parent",_GT_gta_map.fuckup_parent.id)
...
---------------------------------------------------------------------------------------------------------------
_GT_gta_map.fuckup_cp=menu.add_feature("Cayo Perico map","parent",_GT_gta_map.fuckup_parent.id)
...
---------------------------------------------------------------------------------------------------------------
_GT_gta_map.fuckup_cp_cmpnd=menu.add_feature("Cayo Perico Compound","parent",_GT_gta_map.fuckup_parent.id)
...
-- menu.add_feature("display cam rotation","toggle", _G_Options.id, function(f) -- was trying to make my own esp. Trigonomtry is not for me
	-- local me=player.player_id()
	-- local veh_pos
	-- local x_screen = .5
	-- local y_screen
	...
	-- while f.on do
		...
		-- if _GF_valid_veh(veh) then
			-- rot=entity.get_entity_rotation(veh)
		-- else
			-- rot = v3(0,0,0)
		-- end
		...
		-- roty = rot.y
		-- rotz = rot.z
		-- -- dist = _GF_dist_xy_pospos(my_pos,veh_pos)
		-- -- if veh_pos.z >= my_pos.z then
			-- -- y_screen=veh_pos.z-my_pos.z
			-- -- elevation=y_screen
			-- -- y_screen = math.sqrt(dist^2 + y_screen^2)
			-- -- --c 	     = √        (a²     +         b²)
		-- -- else
			-- -- y_screen=my_pos.z-veh_pos.z
			-- -- elevation=y_screen*-1
			-- -- y_screen = math.sqrt(dist^2 + y_screen^2)
			-- -- y_screen = y_screen*-1
		-- -- end
		-- local dir = cam.get_gameplay_cam_rot()
		-- local dirx = dir.x
		-- dirx = _GF_2_decimals(dirx)
		-- local diry = dir.z
		-- diry = _GF_2_decimals(diry)
		-- local text = "v3("
		-- --text=text..dirx..","..diry..")  Distance horizontal:"..math.floor(dist).." Actual Distance:"..math.floor(y_screen).." Elevation difference: "..math.floor(elevation)
		-- text=text..dirx..","..diry..")  Rot x: ".._GF_2_decimals(rotx).." Rot y:".._GF_2_decimals(roty).." Rot z: ".._GF_2_decimals(rotz)
