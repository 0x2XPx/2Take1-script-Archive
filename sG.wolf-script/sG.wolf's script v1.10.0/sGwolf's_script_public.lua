...
--spam
  ...
  menu.add_feature("Chatbot Action", "value_str", chat.id, function (feat)
    if feat.on then
          ...
          for x = 1, #chatbot do
            if player.get_player_name(i.player) == chatbot[x] then
              if feat.value == 0 then
                network.force_remove_player(i.player)
                menu.notify("Kicked Chatbot:" .. chatbot[x], "Chatbot Action", 5, 0xFEE75C)
              else
                --action
              end
            end
      ...
      if chatbot_chat == nil then
        chatbot_chat = event.add_event_listener("chat", function (i)
          local pid, body = i.player, i.body
          if body:find("gta") or body:find("GTA") then
            if chat_repeat[pid] == nil then
              chat_repeat[pid] = {body, 1}
            else
              if chat_repeat[pid][1] == body then
                chat_repeat[pid][2] = chat_repeat[pid][2] + 1
                if chat_repeat[pid][2] == 3 then
                  local name = player.get_player_name(pid)
                  network.force_remove_player(pid)
                  menu.notify("Kicked Chatbot:" .. name, "Chatbot Action", 5, 0xFEE75C)
                end
              else
                chat_repeat[pid] = {body, 1}
              end
    ...
    if not feat.on then
      ...
      event.remove_event_listener("chat", chatbot_chat)
    end

  end):set_str_data({"Kick"})

  local translate_event, target_lang
  menu.add_feature("Translate Chat to", "value_str", chat.id, function(feat)
    if feat.on then
      if feat.value == 0 then
        target_lang = "en"
      ...
      translate_event = event.add_event_listener("chat", function(chat)
        if chat.player ~= player.player_id() and #string.gsub(chat.body, ' ', '') > 1 then
          local array = http_get("https://clients5.google.com/translate_a/t?client=dict-chrome-ex&sl=auto&tl="..target_lang.."&q="..web.urlencode(chat.body))
          if array ~= "Error" then
            
            local table_ = {} --{"translation", "lang"}
            array = string.gsub(array, '""', '"Null"')
            for match in string.gmatch(array, '"([^"]+)"') do
              table.insert(table_, match);
            end

            if target_lang ~= table_[2] then
              if player.is_player_valid(chat.player) then
                menu.notify(player.get_player_name(chat.player) .. ": " .. table_[1], nil, 5)
              end
            end
          else
            menu.notify("Error", "Chat Translater", 5, 0xED4245)
          end
        end
      end)
    end
  ...
  end):set_str_data({"English", "Korean", "Chinese(Traditional)", "Chinese(Simplified)", "French", "Japanese"})
...
menu.add_feature(Translate["check version"], "action", wolf_main_menu.id, function()
  menu.notify("V1.10.0", Translate["sG.wolf's script public ver"], 5, 10)
  menu.notify("sG.wolf", Translate["Developer"], 5, 2)
  menu.notify("kektram, stüssy", Translate["Special thanks to"], 5, 2)
end)
