...
local protex = menu.add_feature(Translate["protex"], "parent", wolf_main_menu.id)
  ...
  local vpn_event
  local detect_vpn_feat = menu.add_feature("Detect VPN", "value_str", protex.id, function (feat)
    if feat.on then
...
local Misc = menu.add_feature(Translate["Misc"], "parent", wolf_main_menu.id)
  ...
  local squid = menu.add_feature(Translate["Squid Game"], "parent", Misc.id)
    ...
    menu.add_feature("Sync in-game time with real-time(beta)", "toggle", Misc.id, function(feat) --translate req
      if feat.on then
        local time_ = utils.time()
...
menu.add_feature(Translate["check version"], "action", wolf_main_menu.id, function()
  menu.notify("V1.8.0", Translate["sG.wolf's script public ver"], 1, 10)
  menu.notify("sG.wolf", Translate["Developer"], 1, 2)
  menu.notify("kektram, stüssy, jhowkNx", Translate["Special thanks to"], 1, 2)
end)
