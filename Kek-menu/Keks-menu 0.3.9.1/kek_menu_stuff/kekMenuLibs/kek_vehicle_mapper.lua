-- Lib vehicle_mapper version: 1.1.2
...
function vehicle_mapper.get_vehicles_from_certain_index(start_index, end_index)
	local vehicles = {}
	for i = start_index, end_index do
		vehicles[#vehicles + 1] = Vehicles[i][1]
	end
	return vehicles
end
...
function vehicle_mapper.GetHashFromModel(model, index, index_2, last_index)
	if not model then
		return 3548084598
	end
	if model == "?" then
		if index and index_2 then
			if last_index then
				index_2 = #Vehicles
			end
			return Vehicles[math.random(index, index_2)][1]
		elseif index then
...
function vehicle_mapper.GetNameFromModel(Model)
...
function vehicle_mapper.GetHashFromName(Name)
...
function vehicle_mapper.GetModelFromName(Name)
	if not Name or Name:len() == 0 then 
		return 
	end
	for i, k in pairs(Vehicles) do
